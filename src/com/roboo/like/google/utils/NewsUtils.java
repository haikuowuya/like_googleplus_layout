...
public class NewsUtils
{
	...
	{
		if (GoogleApplication.mCurrentType.equals(GoogleApplication.TYPE_ITHOME))
		{
			return getITHomeNewsList(baseUrl, pageNo);
		}
		else if (GoogleApplication.mCurrentType.equals(GoogleApplication.TYPE_CSDN))
		{}
		return null;
	}
		...
		if (!majorElements.isEmpty())
		{
			...
			for (int i = 0; i < majorElements.size(); i++)
			{
				...
				if (!elements.isEmpty())
				{
					...
					String title = minorElement.text();
				...
				if (!elements.isEmpty())
				{
					...
					String href = minorElement.attr("href");
				...
				if (!elements.isEmpty())
				{
					...
					minorElement = elements.get(0);
					String img = minorElement.attr("src");
				...
				if (!elements.isEmpty())
				{
					...
					minorElement = elements.get(0);
					String content = minorElement.text();
