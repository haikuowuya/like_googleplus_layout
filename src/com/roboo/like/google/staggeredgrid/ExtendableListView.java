...
package com.roboo.like.google.staggeredgrid;
 ...
 * <p/>
 * This is partly inspired by the incomplete StaggeredGridView supplied in the Android 4.2+ source & the {@link android.widget.AbsListView} & {@link android.widget.ListView} source; however this is intended to have a smaller simplified scope of functionality and hopefully therefore be a workable
 * solution.
 * <p/>
 * Some things that this doesn't support (yet) - Dividers (We don't use them in our Etsy grid) - Edge effect - Fading edge - yuck - Item selection - Focus
 * <p/>
 * Note: we only really extend {@link android.widget.AbsListView} so we can appear to be one of its direct subclasses. However most of the code we need to modify is either 1. hidden or 2. package private So a lot of it's code and some {@link android.widget.AdapterView} code is repeated here Be
 * careful with this - not everything may be how you expect if you assume this to be a regular old {@link android.widget.ListView}
 */
public abstract class ExtendableListView extends AbsListView
{
	...
	public void setAdapter(final ListAdapter adapter)
	{
		...
		{
			mAdapter = new com.roboo.like.google.staggeredgrid.HeaderViewListAdapter(mHeaderViewInfos, mFooterViewInfos, adapter);
		}
		else
		{
			mAdapter = adapter;
	...
	protected void layoutChildren()
	{
		...
		try
		{
			...
			{
				throw new IllegalStateException("The content of the adapter has changed but " + "ExtendableListView did not receive a notification. Make sure the content of " + "your adapter is not modified from a background thread, but only " + "from the UI thread. [in ExtendableListView("
	...
	{
		public static final ListSavedState EMPTY_STATE = new ListSavedState()
		{};
		...
		public ListSavedState(Parcelable superState)
		{
			...
			mClassLoader = AbsListView.class.getClassLoader();
		...
		{
			Parcelable superState = in.readParcelable(mClassLoader);
		...
		protected ListSavedState(Parcelable superState, ClassLoader classLoader)
		{
