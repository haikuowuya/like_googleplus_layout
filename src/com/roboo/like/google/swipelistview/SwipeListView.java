/*
 * Copyright (C) 2013 47 Degrees, LLC http://47deg.com hello@47deg.com Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in compliance with the License. You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0 Unless required
...
public class SwipeListView extends ListView
{
	...
	/**
	 * 当你通过代码创建{@see SwipeListView}时,你需要指定滑动后显示在前面和后面的id
	 * 
	 * @param context
	 *            Context
	 * @param swipeBackView
	...
	private void init(AttributeSet attrs)
	{
		...
		mTouchListener = new SwipeListViewTouchListener(this, swipeFrontView, swipeBackView);
		...
		mTouchListener.setSwipeDrawableChecked(swipeDrawableChecked);
		mTouchListener.setSwipeDrawableUnchecked(swipeDrawableUnchecked);
	...
	{
		mTouchListener.reloadChoiceStateInView(convertView.findViewById(swipeFrontView), position);
	}
	...
	{
		return mTouchListener.isChecked(position);
	}
	...
	{
		return mTouchListener.getPositionsSelected();
	}
	...
	{
		return mTouchListener.getCountSelected();
	}
	...
	{
		mTouchListener.unselectedChoiceStates();
	}
	...
	{
		int height = mTouchListener.dismiss(position);
		if (height > 0)
		{
			mTouchListener.handlerPendingDismisses(height);
		}
		else
		{
			...
			mTouchListener.resetPendingDismisses();
		}
	}
	...
	{
		List<Integer> list = mTouchListener.getPositionsSelected();
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			int auxHeight = mTouchListener.dismiss(position);
			if (auxHeight > 0)
			{
				height = auxHeight;
			}
		...
		mTouchListener.returnOldActions();
	}
